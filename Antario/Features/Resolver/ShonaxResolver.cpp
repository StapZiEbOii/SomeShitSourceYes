...
#include "ShonaxResolver.h"
ShonaxResolver g_Resolver;
...
void ShonaxResolver::AnimationFix(C_BaseEntity* pEnt)
{
	...
	if (storedSimtime[pEnt->EntIndex()] != pEnt->GetSimulationTime())
	{
		...
		if (pEnt != g::pLocalEntity && pEnt->GetTeam() != g::pLocalEntity->GetTeam() && (pEnt->GetFlags() & FL_ONGROUND) && g_Menu.Config.Resolver == 3)
		{
	...
	if (pEnt != g::pLocalEntity && pEnt->GetTeam() != g::pLocalEntity->GetTeam() && (pEnt->GetFlags() & FL_ONGROUND) && g_Menu.Config.Resolver == 3 && jittering[pEnt->EntIndex()])
...
void HandleHitss(C_BaseEntity* pEnt)
{
...
void ShonaxResolver::OnCreateMove() // cancer v2
{
	if (!g_Menu.Config.Resolver == 3)
		return;
...
void ShonaxResolver::FrameStage(ClientFrameStage_t stage)
{
