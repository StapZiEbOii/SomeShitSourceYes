...
void StickResolver::StickAnimationFix(C_BaseEntity* pEnt)
{
	...
	if (storedSimtime[pEnt->EntIndex()] != pEnt->GetSimulationTime())
	{
		...
		{
			float Brute = StickUseFreestandAngle[pEnt->EntIndex()] ? g_Math.NormalizeYaw(Back + StickFreestandAngle[pEnt->EntIndex()]) : pEnt->GetLowerBodyYaw();
		...
		if (pEnt != g::pLocalEntity && pEnt->GetTeam() != g::pLocalEntity->GetTeam() && (pEnt->GetFlags() & FL_ONGROUND) && g_Menu.Config.Resolver == 2)
		{
	...
	if (pEnt != g::pLocalEntity && pEnt->GetTeam() != g::pLocalEntity->GetTeam() && (pEnt->GetFlags() & FL_ONGROUND) && g_Menu.Config.Resolver == 2 && jittering[pEnt->EntIndex()])
...
void StickHandleHits(C_BaseEntity * pEnt)
{
	...
	if (g::Shot[pEnt->EntIndex()])
	{
		...
		{
			g_StickResolver.StickpitchHit[pEnt->EntIndex()] = pEnt->GetEyeAngles().x;
...
void StickResolver::StickOnCreateMove() // cancer v2
{
	...
	for (int i = 1; i < g_pEngine->GetMaxClients(); ++i)
	{
		...
		{
			StickUseFreestandAngle[i] = false;
			continue;
		}

		if (abs(pPlayerEntity->GetVelocity().Length2D()) > 29.f)
			StickUseFreestandAngle[pPlayerEntity->EntIndex()] = false;

		if (abs(pPlayerEntity->GetVelocity().Length2D()) <= 29.f && !StickUseFreestandAngle[pPlayerEntity->EntIndex()])
		{
			...
			for (int side = 0; side < 2; side++)
			{
				...
				if (g_Autowall.CanHitFloatingPoint(OriginAutowall, ViewPoint))
				{
					...
					{
						HitSide1 = true;
						StickFreestandAngle[pPlayerEntity->EntIndex()] = 90;
					}
					else if (side == 1)
					{
						HitSide2 = true;
			...
			if (Autowalled)
			{
				if (HitSide1 && HitSide2)
					...
					StickUseFreestandAngle[pPlayerEntity->EntIndex()] = true;
			}
		}
	}
}

void StickResolver::StickFrameStage(ClientFrameStage_t stage) 
{
	...
	for (int i = 1; i < g_pEngine->GetMaxClients(); ++i)
	{
		...
		{
			StickHandleHits(pPlayerEntity);
			StickAnimationFix(pPlayerEntity);
		}
